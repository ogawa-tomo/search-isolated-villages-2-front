...
describe("Menu", () => {
  it("秘境集落探索、秘境施設占い、このツールについてのリンク", async () => {
    ...
    expect(screen.getByRole("link", { name: "秘境集落探索" })).toHaveProperty(
      "href",
      "http://localhost/",
    );
  ...
  it("秘境施設探索ツールの開閉", async () => {
    ...
    for (const facultyCategory of facultyCategories) {
      expect(
        within(facultySearchDetails).getByRole("link", {
    ...
    for (const facultyCategory of facultyCategories) {
      ...
      ).toHaveProperty("href", `http://localhost/${facultyCategory.pathName}`);
    }
